...
(function () {
  ...
  function rowHtml(nums, teeth) {
    return '<div class="odo-row">' + nums.map(function (n, i) {
      ...
      var bg = color(v);
      var sep = (i === 8) ? ' odo-sep' : '';
      return '<button type="button" class="odo-tooth' + sep + '" data-t="' + n + '" data-v="' + esc(v) + '"' +
  ...
  SP.addSection('dental', {
    ...
    wire: function (el) {
      ...
      el.querySelectorAll('.odo-tooth').forEach(function (b) {
        if (b.dataset.v) teeth[b.dataset.t] = b.dataset.v;
      });
      el._teeth = teeth;
  ...
  function planRow(r) {
    ...
    return '<div class="plan-row">' +
      '<input class="p-tooth" placeholder="رقم السن (26)" value="' + esc(r.tooth || '') + '" />' +
      '<select class="p-treat">' + TREATMENTS.map(function (t) {
        return '<option value="' + esc(t) + '"' + (t === r.treatment ? ' selected' : '') + '>' + (t ? esc(t) : '— العلاج —') + '</option>';
      }).join('') + '</select>' +
      '<select class="p-status">' + STATUSES.map(function (s) {
        return '<option value="' + esc(s) + '"' + (s === r.status ? ' selected' : '') + '>' + esc(s) + '</option>';
      }).join('') + '</select>' +
      '<input class="p-notes" placeholder="ملاحظات" value="' + esc(r.notes || '') + '" />' +
  ...
  SP.addSection('dental', {
    ...
    title: '📋 خطة العلاج',
    html: function (v) {
      var rows = (v && v.length) ? v.map(function (r) { return planRow(r); }).join('') : planRow();
