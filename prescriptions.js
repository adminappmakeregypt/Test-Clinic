...
// كل روشتات العيادة (محلياً + من السحابة) مرتبة بالأحدث
async function listAll() {
  const map = new Map();
  localAll().forEach((x) => map.set(x.id, x));
  try {
    if (auth.currentUser) {
      const snap = await getDocs(collection(db, "clinics", CLINIC_ID, "prescriptions"));
      snap.forEach((d) => map.set(d.id, d.data()));
    }
  } catch (e) {
    console.warn("Prescriptions read from local cache:", e && e.message);
...
// روشتات مريض واحد (بالمفتاح أو الاسم/الجوال)
async function listForPatient(patient) {
  const p = patient || {};
  ...
  return all.filter((rx) => {
    if (p.key && rx.patientKey && rx.patientKey === p.key) return true;
    if (phone && digits(rx.patientPhone) === phone) return true;
    if (!phone && name && (rx.patientName || "").trim().toLowerCase() === name) return true;
    return false;
  });
}
...
function open(visit, patient) {
  ...
  const body = `
    <div class="grid-2 rx-meta">
      ...
      <div><strong>رقم الجوال:</strong> <span>${esc(patient.phone || visit.phone || "-")}</span></div>
      <div class="field"><label>الطبيب</label><input id="rxDoctor" value="${esc(visit.doctor || "")}" placeholder="اسم الطبيب" /></div>
      <div class="field"><label>تاريخ الزيارة</label><input type="date" id="rxVisitDate" value="${esc(visit.appointmentDate || today)}" /></div>
      <div class="field" style="grid-column:1 / -1;"><label>التشخيص</label><input id="rxDiag" value="${esc(visit.condition || "")}" placeholder="التشخيص / الشكوى" /></div>
    </div>
    ${examSectionHtml()}
  ...
  m.el.querySelector("#rxSave").addEventListener("click", async () => {
    ...
    const rx = {
      ...
      doctor: (m.el.querySelector("#rxDoctor").value || visit.doctor || "").trim(),
      visitDate: m.el.querySelector("#rxVisitDate").value || visit.appointmentDate || today,
      issueDate: today,
      diagnosis: (m.el.querySelector("#rxDiag").value || visit.condition || "").trim(),
