...
//     and notifies the open page without forcing a browser reload.
...
function notifyPage(shortKey) {
  // Never reload here. Several Firestore listeners can receive their first
  // snapshot together, and specialty.js may intentionally derive majors from
  // the selected clinic specialty. Reloading for either case creates a loop:
  // boot -> local derived value -> remote merge -> reload -> repeat.
  ...
  SHORT_KEYS.forEach((shortKey) => {
    const un = onSnapshot(
      ...
      (snap) => {
        ...
        if (changed) notifyPage(shortKey);
      },
      (err) => {
