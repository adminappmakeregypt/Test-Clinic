...
(function () {
  ...
  function renderPatients(patients) {
    ...
    patients.forEach(p => {
      ...
      tr.innerHTML = `
        ...
        <td>
          ...
          <button class="btn ghost small dental-model" type="button" data-dkey="${escapeHtml(p.key)}">🦷 Dental model</button>
        </td>
      `;
    ...
    tbody.querySelectorAll('button.dental-model').forEach(btn => {
      btn.addEventListener('click', () => {
        const p = patients.find(x => x.key === btn.getAttribute('data-dkey'));
        if (!p) return;
        if (!window.DentalChart) { alert('جارٍ تحميل وحدة الأسنان، حاول بعد لحظة.'); return; }
        window.DentalChart.open(p);
      });
    });
  ...
  function showDetail(key, patients) {
    ...
    p.visits.forEach(v => {
      ...
      tr.innerHTML = `
        ...
        <td><button class="btn primary small rx-new" type="button" data-visit="${escapeHtml(v.id)}">💊 إنشاء روشتة إلكترونية</button></td>
      `;
      tbody.appendChild(tr);
    });

    tbody.querySelectorAll('button.rx-new').forEach(btn => {
      btn.addEventListener('click', () => {
        const v = p.visits.find(x => String(x.id) === btn.getAttribute('data-visit'));
        if (!v) return;
        if (!window.RX) { alert('جارٍ تحميل وحدة الروشتة، حاول بعد لحظة.'); return; }
        window.RX.open(v, p);
      });
    });
  ...
  async function renderPrescriptions(p) {
    const box = $('#rxListBody');
    const empty = $('#rxEmpty');
    if (!box || !window.RX) return;
    box.innerHTML = '';
    const groups = await Promise.all(p.visits.map(v => window.RX.listFor(v.id)));
    const all = groups.flat();
    all.sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
    all.forEach(rx => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td>${escapeHtml(rx.number)}</td>
        <td>${escapeHtml(rx.issueDate)}</td>
        <td>${escapeHtml(rx.visitDate)}</td>
        <td>${escapeHtml(rx.doctor)}</td>
        <td>${(rx.medications || []).length}</td>
        <td>
          <button class="btn ghost small rx-view" type="button">👁 عرض الروشتة</button>
          <button class="btn primary small rx-pdf" type="button">📄 تحميل PDF</button>
        </td>`;
    ...
    const tbl = $('#rxTable');
    if (tbl) tbl.style.display = all.length ? '' : 'none';
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // refresh the prescriptions list whenever a new one is saved
    const hookRx = () => {
      if (window.RX && !window.RX.__hooked) {
        window.RX.__hooked = true;
        window.RX.onChange(() => { if (currentPatient) renderPrescriptions(currentPatient); });
        if (currentPatient) renderPrescriptions(currentPatient);
      }
    };
